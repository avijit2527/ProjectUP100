...
class RunAgents:
    ...
    def train(self, iterations):
        if(self.training):
            ...
            return self.distance_traveled, np.mean(self.nearest_distance)
...
def plotDiagrams(coverage_array_over_multiple_runs,now,no_of_iter):
    ...
    coverage_array_for_alpha_over_multiple_runs = np.load("./DistanceTraversedVSResponseTime.npy")
    ...
    plt.title("Distance Traversed vs Response Time", fontweight='bold')
    plt.xlabel("Distance Traversed", fontweight='bold')
    ...
    plt.savefig("./Figure/%s/constant_initialization/DistanceTraversedVSResponseTime.png"%(now))
    plt.close() 
        ...
        for num_agents in num_agents_array:
            ...
            for beta_3 in beta_3_array:
                ...
                for itr in iter_array:
                    ...
                    distance_traveled, response_time = game.train(iterations=itr)
                    reward_array.append([distance_traveled, response_time])
