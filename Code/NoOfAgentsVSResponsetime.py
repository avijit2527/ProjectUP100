...
class RunAgents:
    ...
    def setNearestDistance(self,currLoc):
    	temp_min = sys.maxsize;
    	for x in range(self.num_agents):
	        location_ch_1 = self.find_location(x);
	        distance = np.linalg.norm(location_ch_1 - currLoc);
	        temp_min = min(temp_min,distance);
    	self.nearest_distance.append(temp_min);
    ...
    def train(self, iterations):
        if(self.training):
            for i in range(iterations):
                ...
                while (not done) and episode_length < self.max_iter:
                    ...
                    if(random.random() < self.reward_frequncy):
                        ...
                        self.setNearestDistance(temp_reward_state);
            ...
            return np.mean(self.nearest_distance)
...
def plotDiagrams(coverage_array_over_multiple_runs,now,no_of_iter):
    ...
    np.save("NoOfAgents_vs_Responsetime",coverage_array_over_multiple_runs)  



    coverage_array_for_alpha_over_multiple_runs = np.load("./NoOfAgents_vs_Responsetime.npy")
    ...
    #plt.title("Number of Agents vs Response Time", fontsize=16, fontweight='bold')
    ...
    plt.savefig("./Figure/%s/constant_initialization/NoOfAgents_vs_Responsetime.png"%(now))
    plt.close() 
        ...
        for num_agents in num_agents_array:
            ...
            for beta_3 in beta_3_array:
                ...
                for itr in iter_array:
                    ...
                    reward_array.append([num_agents, reward])
